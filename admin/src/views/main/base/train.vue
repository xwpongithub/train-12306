  ...
  <a-modal v-model:visible="visible" title="车次" @ok="handleOk"
    ...
    <a-form :model="train" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="终点站">
        <station-select-view v-model="train.endVal"></station-select-view>
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    watch(() => train.value.endVal, ()=>{
      if (Tool.isNotEmpty(train.value.endVal)) {
        train.value.endPinyin = pinyin(train.value.endVal, { toneType: 'none'}).replaceAll(" ", "");
      } else {
